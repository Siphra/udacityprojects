...
sum(df2.group == 'treatment')/df2.shape[0]
...
for _ in range(10000):
    nsamp = np.random.choice([0,1], dft.shape[0],replace= True, p=[1 - conv,conv])
    osamp = np.random.choice([0,1], dfc.shape[0], replace= True, p=[1 - conv, conv])
    p_diffs.append(nsamp.mean()-osamp.mean())
...
p = (p_diffs[0] > sum(df.converted)/df.user_id.nunique())/10000
p


# k. Please explain using the vocabulary you've learned in this course what you just computed in part **j.**  What is this value called in scientific studies?  What does this value mean in terms of whether or not there is a difference between the new and old pages?

# In part 'j.' of the project we determined the actual p-value for determining if our actual data allowed to us to reject the null hypothesis. In this case, with a p-value of 0.0 we must say that we can not reject the null for this case. 
...
convert_old = sum(dfc.converted)/dfc.shape[0]
convert_new = sum(dft.converted)/dfc.shape[0]
n_old = dfc.shape[0]
n_new = dft.shape[0]
...
pzt([convert_old,convert_new],[n_old,n_new],alternative='larger')


# n. What do the z-score and p-value you computed in the previous question mean for the conversion rates of the old and new pages?  Do they agree with the findings in parts **j.** and **k.**?

# **The z-score is how far removed from the mean of a normal distribution the data is, in our case significantly lower than 1 SD away, meaning it falls in, as the p-value is >> .05 it agrees with the above probability of finding the values outside the expected normal distribution and supports further that we can not reject the null hypothesis in this case.**
...
# **We can see from the above information that even any given nation doesn't have a specific impact on if anyone converts or not, with the best p-val being above our .05 threshold for proof of conversion**
